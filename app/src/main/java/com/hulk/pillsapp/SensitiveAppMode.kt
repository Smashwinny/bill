...
object SensitiveAppMode {
    ...
    fun isSensitivePackage(packageName: String): Boolean = packageName == CMB_PACKAGE

    fun isTargetInstalled(context: Context, packageName: String = CMB_PACKAGE): Boolean =
        context.packageManager.getLaunchIntentForPackage(packageName) != null

    fun pauseForCmb(context: Context): SensitivePauseResult = pause(
        context = context,
        targetPackage = CMB_PACKAGE,
        targetLabel = CMB_LABEL,
    )
    ...
    fun restoreIfExpired(context: Context) {
        ...
        if (session.startedAtMs <= 0L || System.currentTimeMillis() - session.startedAtMs >=
            ...
            // 无法可靠确认银行是否仍在前台，因此只提醒，不盲目重启无障碍。
            SensitiveModeNotifier.showRestoreDue(context, session)
        }
    }

    internal fun markBankLaunched(context: Context, sessionId: String): Boolean =
        synchronized(transitionLock) {
            ...
            if (!SensitivePhaseRules.mayLaunchBank(session.phase, ownServiceAbsent)) {
                return@synchronized false
            }
    ...
    fun notifyUnsafeCmbLaunch(context: Context) {
        SensitiveModeNotifier.showUnsafeLaunch(context)
    }
...
class SensitiveAppLaunchActivity : Activity() {
    private var bankLaunchIssued = false
    private var bankWasForeground = false
    private var restoreStarted = false
    private var sessionId: String? = null

    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        if (!SensitiveAppMode.isTargetInstalled(this)) {
            Toast.makeText(this, "未找到招商银行 App", Toast.LENGTH_LONG).show()
            finish()
            return
        }
        ...
        when (existing?.phase) {
            SensitiveLaunchPhase.PREPARING -> {
                thread(name = "sensitive-mode-preparing-recovery") {
                    ...
                    finishWithToast("安全准备曾被中断，正在恢复行为监视；未启动招商银行")
                }
            }
            SensitiveLaunchPhase.LAUNCHED -> {
                // 进程重建后只有在银行退出、该 Activity 真正恢复时才会走 onPostResume。
                sessionId = existing.id
                bankLaunchIssued = true
                bankWasForeground = true
            }
    ...
    private fun prepareExistingSession(session: SensitiveSession) {
        thread(name = "sensitive-mode-launch") {
            if (!SensitiveAppMode.markBankLaunched(this, session.id)) {
                finishWithToast("安全会话已变化，未启动招商银行")
                return@thread
            }
    ...
    private fun prepareNewSession() {
        thread(name = "sensitive-mode-pause") {
            when (SensitiveAppMode.pauseForCmb(this)) {
                SensitivePauseResult.PAUSED,
                SensitivePauseResult.ALREADY_PAUSED -> {
                    ...
                    if (session == null || session.phase != SensitiveLaunchPhase.PAUSED ||
                        !SensitiveAppMode.markBankLaunched(this, session.id)
                    ) {
                        finishWithToast("安全模式状态不可用，未启动招商银行")
                        return@thread
                    }
                    sessionId = session.id
                    launchBankOnMain()
                ...
                SensitivePauseResult.SETTINGS_WRITE_FAILED ->
                    finishWithToast("无法安全暂停，未启动招商银行")
            }
        }
    }

    private fun launchBankOnMain() {
        runOnUiThread { launchBankOnMainThread() }
    }

    private fun launchBankOnMainThread() {
        if (isFinishing || isDestroyed) return
        val launchIntent = packageManager.getLaunchIntentForPackage(SensitiveAppMode.CMB_PACKAGE)
        bankLaunchIssued = true
        if (launchIntent == null || runCatching { startActivity(launchIntent) }.isFailure) {
            bankLaunchIssued = false
            val expected = sessionId
            thread(name = "sensitive-mode-launch-failure") {
                if (expected != null) SensitiveAppMode.restore(this, expected)
                finishWithToast("招商银行启动失败，正在恢复监视")
            }
        }
    }

    override fun onPause() {
        if (bankLaunchIssued) bankWasForeground = true
        super.onPause()
    }

    override fun onPostResume() {
        super.onPostResume()
        if (!bankWasForeground || restoreStarted) return
...
internal object SensitiveModeNotifier {
    ...
    private const val NOTIFICATION_ID = 19_200
    ...
    fun showPaused(context: Context, session: SensitiveSession): Boolean {
        ...
        val notification = NotificationCompat.Builder(context, CHANNEL_ID)
            ...
            .addAction(0, "立即恢复监视", restore)
            .setOngoing(true)
            .setOnlyAlertOnce(true)
            .setVisibility(NotificationCompat.VISIBILITY_PRIVATE)
            .build()
        return postAndVerify(context, NOTIFICATION_ID, notification)
    }

    @SuppressLint("MissingPermission")
    fun showUnsafeLaunch(context: Context) {
        ...
        val notification = NotificationCompat.Builder(context, CHANNEL_ID)
            ...
            .setContentTitle("招商银行会误报屏幕共享")
            .setContentText("点击安全重开：先暂停行为监视，再进入招商银行")
    ...
    @SuppressLint("MissingPermission")
    private fun postAndVerify(context: Context, id: Int, notification: android.app.Notification): Boolean =
        runCatching {
